...
const int M_PIXEL_END = M_PIXEL_START + M_PIXEL_COUNT; // 40 (index 39)
const int L_PIXEL_START = M_PIXEL_START + M_PIXEL_COUNT; // 40
const int L_PIXEL_END = L_PIXEL_START + L_PIXEL_COUNT;   // 75 (index 74)
...
void light_leds(){
  ...
  for(int pixel = 0; pixel < LED_COUNT; pixel++){
    ...
    // Serial.print("Pixel ");Serial.print(pixel);Serial.print(": Red: ");Serial.println(pixel_colors[pixel][0]);
  }
...
void setup() {
  ...
  Serial.println("Hello. Setup running."); 
...
void loop() {
  ...
  if (adjust_colors) {
      // set the gradient colors
      // todo: adjust colors based on pot input
      int color[2] = {0, 255};     
      set_gradient_color(0, color[random(0, 2)], color[random(0, 2)], color[random(0, 2)]); 
      set_gradient_color(1, color[random(0, 2)], color[random(0, 2)], color[random(0, 2)]);
