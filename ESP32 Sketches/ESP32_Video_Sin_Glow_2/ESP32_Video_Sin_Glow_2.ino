...
void glow(int x, int y, byte d, int f){
  ...
  if(d == 3){ //up glow - triange overflow 
    for(int i = y-1; i>0; i--) {
        ...
        if(c > cmax || c < abs(f)) f = -f;    //make sure color is at least fade amount to always keep values above 0
        c = constrain(c,0,cmax);   
        graphics.drawPixel(x,i,c);
        //Serial.print("c = ");Serial.print((uint8_t)c);Serial.print("  f = ");Serial.println((uint8_t)f);             
    }
  }
...
void draw()
{
  ...
  //draw sine wave accross entire screen
  for(int i = 0; i < XRES; i++){
...
void loop()
{
  ...
  //if(x%10 == 0){
    cmax = cmax + d_cmax;
    if(cmax >= 100 || cmax<20) d_cmax= -d_cmax; //loop cmax between 54 and 200
  //}
