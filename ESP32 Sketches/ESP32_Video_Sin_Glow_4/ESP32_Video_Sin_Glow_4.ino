...
void draw()
{
  ...
  for(int i = 0; i<sin_waves; i++){
    draw_glow_sin(i);
    draw_v_glow_sin(i); //basket weave them if left in same for loop
  }
...
void loop()
{
  ...
  //set variables based on mapped pot values
  //pot 0: cycle length (10-160)
  //pot 1: amplitude (5-20)
  cycle_length = map(potValue[0], 0, 511, 10, 160);
  amplitude = map(potValue[1], 0, 511, 5, 20);

  Serial.print("cycle_length = ");Serial.print(cycle_length);Serial.print("  amplitude = ");Serial.print(amplitude);
  Serial.println();
